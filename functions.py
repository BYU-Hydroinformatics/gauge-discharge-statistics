...
def function_to_create_graphs(str_path_csv_file: str) -> None:
    ...
    # create and save a histogram and boxplot for each column
    for column in df.columns:
        df[column].plot.hist(bins=100)
        plt.savefig(os.path.join('graphs', column + '_histogram.png'))
        plt.close()

        df[column].plot.box()
        plt.savefig(os.path.join('graphs', column + '_boxplot.png'))
        plt.close()
